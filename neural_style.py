...
def stylize(args):
    ...
    #cap.open("rtsp://admin:gist2406@192.168.80.241/video1+audio1") # 싸움터
    #cap.open("rtsp://admin:gist2406@192.168.190.241/video1+audio1") #
                ...
                while(cap.isOpened()):
                    ...
                    try:
                        if count % 2 == 0:
                            ...
                            img = output[0].clone().cpu().clamp(0, 255).cpu().numpy()
                            img = img.transpose(1, 2, 0).astype("uint8")
                            # print(np.shape(img))

                            # Multi-Window
                            img = cv2.resize(img, (640, 480))
                            numpy_horizontal_concat = np.concatenate((frame1, img), axis=1)
